    ...
    if LpImg:
        ...
        for i in number:
            if i.isalnum() and (not i.islower()):
                s.append(i)
        v_number = "".join(s)
        csv_updater(v_number)  # Update CSV with vehicle number
        
        # Check if the plate number exists in the Indian car dataset
        indian_car_dataset = load_indian_car_dataset('car_data.csv')
        allow = v_number in indian_car_dataset
        return v_number, img_tosave, allow
    else:
        return 0, 0, False
