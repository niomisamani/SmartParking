...
def load_indian_car_dataset(filename):
    dataset = set()
    with open(filename, 'r', newline='') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            dataset.add(row['Car Number Plate'])
    return dataset
...
# Update the csv_updater function to check the length of the vehicle number before adding it to the CSV
import re

# Update the csv_updater function to enforce the format constraint on the vehicle number
# Initialize an empty list to store the results
results = []

# Update the csv_updater function to store the results and print the final result
def csv_updater(v_number):
    if re.match(r'^[A-Za-z]{2}\d{2}[A-Za-z]{2}\d{4}$', v_number):  # Check the format using a regular expression
        ...
        indian_car_dataset = load_indian_car_dataset('car_data.csv')
        
        # Check if the number plate exists in the Indian car dataset
        if v_number in indian_car_dataset:
            results.append("Allow")  # Store "Allow" in the results list
        else:
            results.append("Reject")  # Store "Reject" in the results list
        
        f.close()
    else:
        results.append("Reject: Vehicle number format is not valid")  # Store "Reject" in the results list

# After processing all records, print the final result
